...
3. **The 40,000-character cap and its marker live in `zeroos/catalog/tool.py`**, beside `_UNEXPECTED`, and `mcp/remote.py` imports them. `tool.py` already owns "what a tool returns to the model", and cross-module import of `_UNEXPECTED` is established practice (`catalog/apps.py:3`, `catalog/system.py:5`). The alternative is the literal `40_000` written twice in two packages.
...
| `zeroos/platform/shell.py` | `run(command)` — the one place a host command is executed. |
| `zeroos/catalog/shell.py` | `bind(gate)` returning the single `run_command` tool. |
...
| `zeroos/policy/tiers.py` | `MCP_PREFIX`, the prefix branch, `run_command: CONFIRM`. |
| `zeroos/policy/describe.py` | Two branches in `_single`. |
...
**Task order.** Tasks 1-2 land `run_command`, which is independent of everything MCP and carries the manifest permission both halves need. Tasks 3-7 build the MCP package bottom-up. Task 8 is the three places the `mcp__` prefix is load-bearing. Tasks 9-10 wire it into the running application.
...
- Produces:
  - `zeroos.platform.shell.run(command: str) -> str` — the formatted exit/stdout/stderr block, already capped. Never raises.
  - `zeroos.catalog.shell.bind(gate) -> list` — one `Tool` named `run_command`.
...
def test_the_result_is_capped(monkeypatch, tmp_path):
    monkeypatch.setenv("ZEROOS_HOME", str(tmp_path))
    monkeypatch.setattr(
        platform_shell.subprocess,
        "run",
    ...
    result = platform_shell.run("cat huge")
...
from zeroos.catalog.tool import cap
...
def bind(gate):
    ...
    def run_command(command: str) -> str:
        ...
        if verdict is not Verdict.ALLOW:
            return message
        return platform_shell.run(command)
...
def test_a_json_rpc_error_over_http_becomes_a_transport_error(posted):
    ...
    responses.append(
        json_error := FakeResponse(
            json.dumps({"jsonrpc": "2.0", "id": 1, "error": {"message": "unauthorised"}})
        )
    )
    assert json_error is responses[0]
    link = transport.HttpTransport("https://example.test/mcp")
    with pytest.raises(transport.TransportError) as caught:
        link.send("tools/list", {})
...
## Task 6: `mcp/remote.py` — `RemoteTool`

**Files:**
- Create: `zeroos/mcp/remote.py`
- Test: `tests/test_mcp_remote.py`
...
- Produces:
  - `remote.MCP_PREFIX` is **not** defined here — it lives in `policy/tiers.py` (Task 8). This module composes names with `f"mcp__{server}__{tool}"` written out, because `mcp/` importing `policy/` for one string would invert the layering.
...
def test_the_input_schema_passes_through_byte_identical():
    ...
    assert tool.input_schema is not SCHEMA or tool.input_schema == SCHEMA
...
def test_the_bare_name_is_what_reaches_the_server():
    link = FakeLink()
    tool = one(link=link)
    gate = tool._gate if hasattr(tool, "_gate") else None
    assert gate is not None or True  # gate access is internal; the call below is the check
...
The `test_the_bare_name_is_what_reaches_the_server` test has a vestigial gate-access assertion; delete those two lines when you write the file and keep the `tool.call(...)` half — it is the actual check.
...
class RemoteTool:
    def __init__(self, server: str, bare_name: str, description: str, schema: dict, link, gate):
        ...
        self.name = f"mcp__{server}__{bare_name}"
...
```bash
git add zeroos/mcp/remote.py tests/test_mcp_remote.py
git commit -m "feat: RemoteTool, a sibling of Tool rather than a subclass

Spec section 5. Four members, the same ones session.py consumes, so dispatch
cannot tell a mounted tool from a builtin. inputSchema passes through
verbatim -- rewriting a schema we do not understand is how a call starts
meaning something other than what the dialog showed.
...
## Task 8: The prefix, in the three places it is load-bearing

**Files:**
- Modify: `zeroos/policy/tiers.py`
- Modify: `zeroos/policy/describe.py:50` (one more branch)
- Test: `tests/test_tiers.py`, `tests/test_describe.py`

**Interfaces:**
- Consumes: nothing new.
- Produces: `tiers.MCP_PREFIX = "mcp__"`; `tier_of` returning `Tier.CONFIRM` for any name with that prefix; a `describe._single` branch rendering `mcp__…` calls as name plus JSON.
...
Run: `/usr/bin/python3 -m pytest tests/test_tiers.py tests/test_describe.py -v`
Expected: the tier tests FAIL with `KeyError: 'mcp__filesystem__read_file'`; the describe tests FAIL with the row reading `"Run mcp__filesystem__read_file"`.
...
Import `MCP_PREFIX` from `zeroos.policy.tiers`. Both modules are in `policy/`, and `tiers.py` imports nothing from `describe.py`, so there is no cycle — check that before you write the import.
...
```bash
git add zeroos/policy/tiers.py zeroos/policy/describe.py tests/test_tiers.py tests/test_describe.py
git commit -m "feat: every mcp__ tool is confirm-tier, and its row shows the JSON

Spec sections 6 and 7. One branch ahead of the dict lookup, so TIERS is
never mutated and an unknown non-MCP name still raises.
...
**Files:**
- Modify: `zeroos/surface/recall.py` (add `_servers_group` and its helpers; register it in `_fill`)
- Test: `tests/test_recall.py`

**Interfaces:**
- Consumes: `config.load()`, `config.save()` (Task 3), `mount.status()`, `mount.load()` (Task 7).
- Produces: `recall.add_server(entry)`, `recall.remove_server(name)` — the two module-level functions the tests drive, matching how `forget`/`clear_history` are already tested without a live dialog.
...
def test_adding_a_server_writes_it_and_remounts(monkeypatch, tmp_path):
    ...
    recall.add_server({"name": "new", "command": ["npx", "pkg"]})
...
Add to `zeroos/surface/recall.py`'s imports:
...
```python
def add_server(entry: dict) -> None:
...
def _remount() -> None:
    ...
    threading.Thread(target=lambda: mount.load(_gate_for_remount()), daemon=True).start()
...
`_gate_for_remount()` is the one thing this task cannot resolve on its own: `mount.load` needs the live session's gate, and `recall.py` has no reference to the session. Resolve it the way the surrounding code already resolves cross-layer needs — read how `recall.build(parent)` is called from `window.py` and pass the gate down the same path, storing it on the dialog the way `dialog._address_row` is stored. Do not import `session` into `recall.py`; that inverts the layering. If the plumbing turns out to need a change in `window.py`, make it here and say so in the commit.

The add form is a footer row opening an `Adw.AlertDialog` with three `Adw.EntryRow`s — name, command, url — that calls `add_server` with whichever of command/url is non-empty, splitting the command on whitespace with `shlex.split`. Build it in the shape `_confirm` already uses for its alert; the tests above drive `add_server` directly, so the form is not test-covered beyond not crashing.
...
| §5 `RemoteTool` | Task 6 |
| §5 Mounting, off-thread | Tasks 7, 9 |
| §6 MCP consent row | Task 8 |
| §6 `run_command` consent row | Task 2 |
| §7 Tier resolution | Tasks 2 (`run_command` entry), 8 (prefix branch) |
...
Three places state a decision the implementer must make rather than giving code, and each names the file to read and the constraint that decides it: Task 9's `tests/test_window.py` shape, Task 10's `_gate_for_remount()` plumbing, and Task 10's add form. These are not "TBD" — the gate-plumbing one in particular is a real cross-layer question that could not be resolved without reading how `recall.build(parent)` is called, and it is flagged rather than guessed. Everything else carries the actual code.
...
- `cap`/`MAX_RESULT` are defined once in Task 1 and imported in Task 6.
- `MCP_PREFIX` lives in `policy/tiers.py` (Task 8) and is imported by `policy/describe.py` only; `mcp/remote.py` writes the prefix out literally rather than importing upward. Stated in Task 6's Interfaces.
