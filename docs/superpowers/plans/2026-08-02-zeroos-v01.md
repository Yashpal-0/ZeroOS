...
- Produces:
  ...
  - `zeroos.policy.gate.Gate(ask)` where `ask: Callable[[list[str]], list[bool]]` takes the description rows and returns one boolean per row.
...
def test_confirm_tier_without_prepare_fails_closed_by_asking(home):
...
class Gate:
    ...
    def prepare(self, calls: list[tuple[str, dict]]) -> None:
        ...
        # A missing answer is a denial. Short-circuiting to DENY here is what
        # makes a dialog that was dismissed, crashed, or answered partially
        # fail closed rather than open.
        for row_number, (_, covered) in enumerate(rows):
            approved = bool(answers[row_number]) if row_number < len(answers) else False
    ...
    def decide(self, name: str, arguments: dict) -> tuple[Verdict, str]:
        ...
        # Nothing prepared for this call. Fail closed.
        try:
            tier = tier_of(name)
        except KeyError:
...
git commit -m "$(cat <<'EOF'
feat: batched approval gate with fail-closed enforcement

Decides once per turn from the full call list so the user sees one dialog,
enforces inside each tool function so an unprepared call still cannot run.
