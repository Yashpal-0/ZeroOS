...
testpaths = ["tests"]
pythonpath = ["."]
# pytest's default tmp_path lives under /tmp, which is tmpfs on most desktops.
# Gio.File.trash() refuses to trash on system internal mounts, so the tests that
# prove ZeroOS moves files to the trash rather than deleting them — the product's
# central promise — cannot run there. Keeping scratch files on the same real
# filesystem as the repo lets them actually exercise it.
...
`pythonpath = ["."]` replaces an editable install: Ubuntu 26.04 is PEP 668
externally-managed and `python3.14-venv` is absent, so `pip install -e` cannot
run. pytest supports this natively and tests run from the repo root with no
install step. `.pytest-tmp/` belongs in `.gitignore`.
...
```bash
git add zeroos/catalog/ tests/test_tool.py tests/test_catalog_files.py
git commit -m "$(cat <<'EOF'
...
Run: `python -m pytest tests/test_catalog_openers.py -v`
Expected: 16 passed (9 test functions; the two parametrized ones expand to 4 and 5 cases)
