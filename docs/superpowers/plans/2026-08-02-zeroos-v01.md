...
**Interfaces:**
- Consumes: `zeroos.catalog.registry.build`, `zeroos.policy.gate.Gate`, `zeroos.agent.prompt.SYSTEM_PROMPT`, `zeroos.agent.log.record`, `zeroos.policy.tiers.tier_of`.
- Produces:
...
def test_the_loop_is_bounded(home):
    # A model that never stops calling tools must not hang the app.
    responses = [
        FakeMessage(tool_calls=[tool_call(str(n), "list_folder", path=str(home / "Downloads"))])
        for n in range(session_module.MAX_STEPS + 5)
    ]
    session, _, client = build_session(responses, [])
    session.send("loop forever")
    assert len(client.requests) <= session_module.MAX_STEPS
```
...
# ponytail: a turn that needs more than eight round trips is a stuck model,
# not a hard task. Raise it if a real workload proves otherwise.
MAX_STEPS = 8
...
class Session:
    ...
    def _run(self, name: str, arguments: dict) -> str:
        ...
        log.record(name, arguments, tier_of(name).value, "executed", result)
        return result
